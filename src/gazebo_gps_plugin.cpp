...
namespace gazebo
{
  ...
  void GpsPlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
  {
    ...
    {
      ros_topic_ = _sdf->GetElement("ros_topic")->Get<std::string>();
    }
    else
    {
      ros_topic_ = "/gazebo_ros_gps";
      gzwarn << "[gazebo_gps_plugin] Using default ros topic name of "
             << ros_topic_ << "hz \n";
    }
    ...
    ros_pub_ = rosnode_->advertise<sensor_msgs::NavSatFix>(ros_topic_, 1);
  }
  ...
  void GpsPlugin::OnSensorUpdate()
  {
    ...
    if ((current_time_ - last_gps_time_).Double() > 1 / parentSensor_->UpdateRate())
    {
      ...
      ros_msg_.latitude = gps_msg.latitude_deg();
      ros_msg_.longitude = gps_msg.longitude_deg();
      ros_msg_.altitude = gps_msg.altitude();
